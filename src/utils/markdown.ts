...
export function iconPill(icon: keyof typeof Emojis, content: any) {
  return (
    _icon(icon) + "**` " + _escapeCodeblock(content).replace(/ /g, " ") + " `**"
  );
}
...
export function stringwrapPreserveWords(
  content = "",
  lenght: number,
  newLines = true,
) {
...
export function inline(cols: string[][], options?: { spacing?: number }) {
  ...
  const lengths = cols[0]!.map((_, colIndex) =>
    Math.max(...cols.map((row) => (row[colIndex] ?? "").length)),
  );

  let lines = cols.map((row) =>
    row
